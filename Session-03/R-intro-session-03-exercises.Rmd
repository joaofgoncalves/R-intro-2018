...
Lists are very flexible and, in fact, many outputs from R functions are formatted as _'list-like'_ object (although often these have specific classes). This is one of the reasons why learning to work with lists and know how to access their content is so important.
...
a) Make a function called `Celsius2Kelvin` that converts temperature from Celsius to Fahrenheit (formula: T(K) = T(°C) + 273.15). Test it with `a <- 20.5`.
...
b) Create a function that checks if the Spearman correlation between two vectors (`x` and `y`) is higher than |r| > 0.7 (absolute value) and returns `TRUE` if it verifies that condition. Use `cor.test()` to calculate the correlation (hint: use list indexation in the `cor.test()` function);
...
b) Do a function that takes a __vector__ (named `x`) and a __matrix__ (named `y`) to calculate the correlation and prints the results __for each column__ in the matrix. Use function `cor.test` to calculate the Pearson correlation. If the correlation is higher than |rho| > 0.7 then print the result using `summary`. For testing the function, use `airquality$Ozone` as `x` and a matrix with all of the remaining columns from `airquality` as `y`.
